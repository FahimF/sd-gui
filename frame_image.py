...
class ImageFrame(BaseFrame):
	def __init__(self, parent, cfg, *args, **kwargs):
		...
		self.m_lbl_input = tk.Label(self, text='Input Image', font=lbl_font)
		self.m_lbl_input.grid(row=0, column=1, padx=(8, 8), pady=(4, 2), sticky='W')
		# Input image entry
		self.m_txt_input = tk.Entry(self, textvariable=self.init_image, state=tk.DISABLED)
		self.m_txt_input.grid(row=1, column=1, padx=(8, 4), pady=(2, 4), sticky='EW')
		# Input image button - to show open dialog
		self.m_btn_input = tk.Button(self, text='...', command=self.pick_image)
		self.m_btn_input.grid(row=1, column=2, padx=(4, 8), pady=(2, 4), sticky='W')
		# Input image preview
		self.m_input_image = tk.Canvas(self, width=128, height=128, bg='lightgrey')
		self.m_input_image.grid(row=1, column=3, rowspan=2, padx=8, pady=(4, 4))
		# Left frame
		self.m_left = tk.Frame(self)
		self.m_left.grid(row=3, column=0, sticky='NS')
		# Right frame
		self.m_right = tk.Frame(self)
		self.m_right.grid(row=3, column=1, columnspan=3, sticky='NSEW')
		# Scheduler label
		...
		# Main - Separator
		ttk.Separator(self, orient='horizontal').grid(row=6, column=0, columnspan=4, pady=8, sticky='EW')
		# Generate button
		self.m_generate = tk.Button(self, text="Generate!", command=self.generate_images)
		self.m_generate.grid(row=7, column=0, columnspan=4)
		# Resize handling
