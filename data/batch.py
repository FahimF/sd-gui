		...
		try:
			...
			if self.id == -1:
				...
				sql = f'INSERT INTO "batches" ("prompt_id, scheduler, width, height, inference_steps, guidance_scale, num_copies, ' \
					  f'time_taken, seed, noise_strength") VALUES ({self.prompt_id}, {self.scheduler}, {self.width}, {self.height}, ' \
					  f'{self.inference_steps}, {self.guidance_scale}, {self.num_copies}, {self.time_taken}, {self.seed}, ' \
					  f'{self.noise_strength})'
			else:
				sql = f'UPDATE "batches" SET "prompt_id" = {self.prompt_id}, scheduler = {self.scheduler}, width = {self.width}, ' \
					  ...
					  f'noise_strength = {self.noise_strength} WHERE "id" = {self.id}'
